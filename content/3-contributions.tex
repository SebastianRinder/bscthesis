...
where $\KL(.||.)$ denotes the Kullback Leibler divergence $\mathcal{H}(p) = -\int p(\x)\log(p(\x)) d\x$ is the entropy of $p$.
...
When doing hyper parameter optimization we do not double the noise variance. Instead our log marginal likelihood function (\ref{eq:hypOpt}) returns -infinity for hyper parameters, which produce a non positive definit matrix $K_n$. Therefore, such hyper parameters do not come into consideration when maximizing.
